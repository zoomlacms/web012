<%@ page language="C#" autoeventwireup="true" inherits="manage_3Dmanage_UserManage, App_Web_3ritkdm2" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE HTML>
<html>
		...
		<asp:Repeater ID="Repeater1" runat="server">
			<ItemTemplate>
				...
				<%--    <%#GetUserName(Eval("Duid").ToString()) %>--%>
					   <span style="cursor:hand" onmouseover="showmenu(event,'<div class=menuitems style=float:left;><a href=ADDUser.aspx?menu=edit&id=<%#Eval("Duid") %>&Duserid=<%#Eval("Duserid") %>  target=main_right>修改</a></div><div  class=menuitems style=float:left;><a href=UserManage.aspx?menu=del&id=<%#Eval("Duid") %>  target=main_right>删除</a></div>',0,15)"><%#GetFiled(Eval("Duid"),"username")%></span>
