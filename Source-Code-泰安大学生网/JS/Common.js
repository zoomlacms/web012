...
function DealwithUploadPic(path, id) {
    if (document.getElementById(id).value.indexOf("|") > -1) {
        ...
        if (path.indexOf('|') > -1) {
            ...
            document.getElementById(id).value = cs[0] + "|" + temppath[1];
        }
        else {
            document.getElementById(id).value = cs[0] + "|" + path;
        }
    } else {
...
function SelectThumbFiles(thumbID) {
    ...
    var sValue = dir.match(new RegExp("[\?\&]NodeID=([^\&]*)(\&?)", "i"));
    var urlstr = "../Common/ShowUploadFiles.aspx" + (sValue ? "?NodeID=" + sValue[1] : "");
    var isMSIE = (navigator.appName == "Microsoft Internet Explorer");
    var arr = "";
    if (isMSIE) {
        arr = window.showModalDialog(urlstr, "self,width=400,height=300,resizable=yes,scrollbars=yes");
        if (arr != null) {
...
/////////////////////////////////////////////////////////
function AddJs(txt1, txt2, rbl) {
    var text1 = document.getElementById(txt1).value;
    var text2 = document.getElementById(txt2);
    var rdo = document.getElementsByName(rbl);
    var a = "";
    if (text1 != null && text1 != "") {
        for (var i = 1; i < rdo.length; i++) {
            if (rdo[i].checked){
                a = rdo[i].value;
            }
        }
        if (a == "JS") {
            text2.value = text1 + ".js";
        }
        else if (a == "HTML") {
            text2.value = text1 + ".html";
        }
    }
}