<%@ page language="C#" autoeventwireup="true" inherits="User_UserZone_Cut_ModulePcuting, App_Web_k3p0ohfx" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE html >
<html>
<head>
<style type="text/css">
...
<body>
    ...
    <div id="divmain" style="width: 100%; height: 1000px; overflow:hidden;  position: relative">
        ...
        <div id="InputTitle" style="position: absolute; top: 100px; z-index:1000; left: 100px; width: 400px; height: 30px; background-color: #FCC; vertical-align: middle; padding-top: 5px; padding-bottom: 2px; padding-left: 5px; display: none;">
            <select id="widgetClass">
            ...
            <input id="webtitle" onfocus="this.value=='输入标题'?this.value='':0" type="text" onblur="this.value==0?this.value='输入标题':0" value="输入标题" />
    ...
    <script type="text/javascript">
        ...
        function onsaveiframe() {
            //divsx = divs.style.left.substr(0, divs.style.left.length - 2);
            // divsy = divs.style.top.substr(0, divs.style.top.length - 2);
            var title = webtitle.value.length == 0 ? "请输入标题" : webtitle.value;
            //var pageId = document.getElementById("pageId").innerText;
            var wcs = document.getElementById("widgetClass").selectedIndex; //获取选择组件类型值
            var url = "ModulePcutmanage.aspx?top=" + divsy + "&menuid=0&left=" + divsx + "&width=" + divw + "&height=" + divh + "&title=" + escape(title) + "&url=" + escape(iframeMain.src) + "&ClassID=" + wcs;
            //   AjAXflag = false;
            //  tjAjax(url);
