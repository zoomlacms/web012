...
$(function(){
    $("#GaoHtml").bind("click",function(){
		...
		$("#popupboxs").append("<div id='boxCovera' style='text-align: center;'>网址：<input id='cURL' style='width: 200px;' value='http://' />"
		...
		$("#btnoper").click(function(){
			 var url = $("#cURL").val();
			 //$("#cOK").attr("display",block);
			 
			if( typeof($("#boxMain")[0])!="undefined")
			{
				 $("#popupboxs").remove($("#boxMain"));
			}
			
		   $("#popupboxs").append("<div id='boxMain' class='box_main'><iframe id='box_url' frameborder='0' " +
				" marginwidth='0' marginheight='0' width='100%' height='100%' " +
				" src='ModulePcutGao.aspx?url=" + url + "'></div>");
		})
        // $("#popupbox2").show();
